...
<div class="mx-auto max-w-3xl page-transition space-y-6">
    <div>
        <h1 class="text-3xl font-semibold text-slate-900 dark:text-white">Nuevo usuario</h1>
        <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Alta base para la administracion interna.</p>
    </div>

    <form method="post" action="/configuracion/usuarios/guardar" class="glass-card space-y-6 p-8">
        <?= csrf_field() ?>
        <div class="grid gap-6 md:grid-cols-2">
            <label class="space-y-2">
                <span class="text-sm font-medium">Usuario</span>
                <input name="username" value="<?= old('username') ?>" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900" required>
            </label>
            <label class="space-y-2">
            ...
            <label class="space-y-2 md:col-span-2">
                ...
                <select name="role" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900" required>
                    <?php foreach ($roles as $value => $label): ?>
                        <option value="<?= esc($value) ?>" <?= old('role', 'operator') === $value ? 'selected' : '' ?>><?= esc($label) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <label class="inline-flex items-center gap-3 text-sm font-medium">
                <input type="checkbox" name="active" value="1" class="h-5 w-5 rounded border-slate-300 text-slate-900" <?= old('active', '1') ? 'checked' : '' ?>>
